...
inline
unsigned getUTF8StringLength(const std::string &s) {
   unsigned retval=0;
   unsigned idx=0;
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         LOG(FATAL) << "in utf.h getUTF8StringLength: std::string '" << s << "' not encoded in unicode utf-8";
         return retval;
      }
   }
   if (idx != s.length()) {
...
inline int getCharactersFromUTF8String(const std::string &s, CSentence *sentence) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         LOG(FATAL) << "in utf.h getCharactersFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8";
         sentence->push_back("?");
         ++len;
         ++idx;
      }
   ...
   if (idx != s.length()) {
      LOG(FATAL) << "in utf.h getCharactersFromUTF8String: std::string '" << s << "' not encoded in utf-8" ;
      return len;
   }
...
inline std::string getFirstCharFromUTF8String(const std::string &s) {
   ...
   if ((s[0]&0x80)==0) {
      ...
      LOG(FATAL) << "in utf.h getFirstCharFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8";
      return "?";
   }
}
...
inline std::string getLastCharFromUTF8String(const std::string &s) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         LOG(FATAL)<< "in utf.h getLastCharFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8";
         return "?";
      }
   }
   if (idx != s.length()) {
